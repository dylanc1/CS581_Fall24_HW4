/*
    ...
    Homework #4
    Compile with: mpicc -g -Wall -o hw4-v2 hw4-v2.c
    Run: mpiexec -n [threads] ./hw4-v2 [side length] [iterations] [output directory]
    Note: This program is the version that uses non-blocking calls
*/
...
void compute_local( // local_x, n,counts,my_rank,comm_sz,comm
   ...
   /* Initialize the boundaries of the life matrix */
    ...
    for (k = 0; k < iterations; k++) {
        ...
        if ((my_rank % 2) == 0) {
            /* exchange up */
            MPI_Irecv(&(life[nRows+1][0]), nColsGhost, MPI_INT, upper_rank, 0, comm, &r1);
            MPI_Isend(&(life[nRows][0]), nColsGhost, MPI_INT, upper_rank, 0, comm, &s1);
        }
        else {
            /* exchange down */
            MPI_Irecv(&(life[0][0]), nColsGhost, MPI_INT, down_rank, 0, comm, &r1);
            MPI_Isend(&(life[1][0]), nColsGhost, MPI_INT, down_rank, 0, comm, &s1);
        }

        /* Do the second set of exchanges */
        if ((my_rank % 2) == 1) {
            /* exchange up */
            MPI_Irecv(&(life[nRows+1][0]), nColsGhost, MPI_INT, upper_rank, 1, comm, &r2);
            MPI_Isend(&(life[nRows][0]), nColsGhost, MPI_INT, upper_rank, 1, comm, &s2);
        }
        else {
            /* exchange down */
            MPI_Irecv(&(life[0][0]), nColsGhost, MPI_INT, down_rank, 1, comm, &r2);
            MPI_Isend(&(life[1][0]), nColsGhost, MPI_INT, down_rank, 1, comm, &s2);
        }
